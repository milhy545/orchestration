    ...
    try:
        if request.operation == "create":
            if not request.collection_name or not request.vector_size:
                raise HTTPException(
                    status_code=400, detail="Collection name and vector size required"
                )
    ...
    try:
        if request.operation == "insert":
            ...
            if not request.point_id or not request.vector:
                raise HTTPException(
                    status_code=400, detail="Point ID and vector required for update"
                )
            ...
            result = await qdrant_client.upsert(
                collection_name=request.collection_name, points=[point]
            )
...
async def search_tool(request: SearchRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        for result in results:
            result_data = {"id": result.id, "score": result.score}
