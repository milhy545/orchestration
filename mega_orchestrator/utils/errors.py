...
class MCPConnectionError(MCPError):
    ...
    def __init__(self, service_name: str, original_error: Optional[Exception] = None):
        ...
        super().__init__(f"Failed to connect to MCP Service '{service_name}'.", code=503, details=details)
...
def handle_exception(e: Exception, context: str = "") -> Dict[str, Any]:
    ...
    if isinstance(e, MCPError):
        logger.error(f"MCP Error in {context}: {e.message} (Code: {e.code})", extra={"details": e.details})
        return {
            "status": "error",
            "error": {
                "message": e.message,
